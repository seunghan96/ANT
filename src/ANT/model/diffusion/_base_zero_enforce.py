...
class TSDiffBase(pl.LightningModule):
    def __init__(
        ...
        self.zero_enforce = zero_enforce
        ...
        self.lr = lr
        self.local_best_crps = dict()
        self.global_best_crps = np.inf
    ...
    def p_losses(
        ...
        predicted_v = self.backbone(x_noisy, t, features)
        
        alphas_t = extract(self.alphas, t, x_noisy.shape)
        posterior_variance_t = extract(self.posterior_variance, t, x_noisy.shape)
        target = alphas_t*noise - x_noisy* torch.sqrt(posterior_variance_t)
        #print(target)
        ...
        #adfsd
        if loss_type == "l1":
            loss = F.l1_loss(target, predicted_v, reduction=reduction)
        elif loss_type == "l2":
            loss = F.mse_loss(target, predicted_v, reduction=reduction)
        elif loss_type == "huber":
            loss = F.smooth_l1_loss(
                target, predicted_v, reduction=reduction
            )
        else:
            raise NotImplementedError()

        return loss, x_noisy, 777

    @torch.no_grad()
    def p_sample(self, x, t, t_index, features=None):
        alphas_t = extract(self.alphas, t, x.shape)
        posterior_variance_t = extract(self.posterior_variance, t, x.shape)
        predicted_v = self.backbone(x, t, features)
        model_mean = (alphas_t*x) - predicted_v* torch.sqrt(posterior_variance_t)
        #print(model_mean)
        #adfs
        return model_mean
    ...
    def sample(self, noise, features=None):
        ...
        #print(np.stack(seqs, axis=0))
        #afdsfs
        return np.stack(seqs, axis=0)

    def fast_denoise(self, xt, t, features=None, noise=None):
        alphas_t = extract(self.alphas, t, xt.shape)
        predicted_v = self.backbone(xt, t, features)
        posterior_variance_t = extract(self.posterior_variance, t, xt.shape)
        model_mean = (alphas_t*xt) - predicted_v* torch.sqrt(posterior_variance_t)
