...
def main(config: dict, log_dir: str, samples_path: str):
    ...
    if os.path.exists(log_dir):
        ...
        if config['lr_scale']==1:
            PATH = f'results_best_deepar_seed{random_seed}.yaml'
            if config['mm']:
                PATH = PATH.replace('.yaml','_mm.yaml')
                
        else:
            PATH = f'results_best_deepar_seed{random_seed}_lrs{config["lr_scale"]}.yaml'
            if config['mm']:
                PATH = PATH.replace('.yaml','_mm.yaml')
    ...
    if samples_path is None:
        ...
        np.save(log_dir / f"synth_samples_best_seed{random_seed}.npy", synth_samples)
    else:
