...
def main(config: dict, log_dir: str, samples_path: str):
    ...
    if os.path.exists(log_dir):
        import sys
        PATH = f'results_last_seed{random_seed}.yaml'
        if config['mm']:
            PATH = PATH.replace('.yaml','_mm.yaml')
        PATH = PATH.replace('.yaml',f"_da{config['dataset_alpha']}.yaml")
        if config['ma']!=0:
            PATH = PATH.replace('.yaml',f"_ma{config['ma']}.yaml")
        if config['temp2']!=1:
            PATH = PATH.replace('.yaml',f"_temp2{config['temp2']}.yaml")
        if config['two_side']:
            PATH = PATH.replace('.yaml',f"_ts.yaml")
    ...
    if config['ma']==0:
        kk = log_dir / f"synth_samples_last_seed{random_seed}_da{config['dataset_alpha']}.npy"
    else:
        kk = log_dir / f"synth_samples_last_seed{random_seed}_da{config['dataset_alpha']}_ma{config['ma']}.npy"
    print(kk)
    ...
    if samples_path is None:
        ...
        try:
            ...
            np.save(log_dir / f"synth_samples_last_seed{random_seed}_da{config['dataset_alpha']}.npy", synth_samples)
    '''
    def calc_effective_N(acf,abs_=False):
        if abs_:
            ratio = 1+(np.abs(acf).sum()*2)-2
        else:
            ratio = 1+(acf.sum()*2)-2 
            
        effective_N = len(acf)/ratio
        return effective_N,ratio

    from statsmodels.tsa.stattools import acf

    X_real_iat = []
    for x in synth_samples:
        acf_values = acf(x, fft=False, nlags=len(x)-1)
        X_real_iat.append(calc_effective_N(acf_values,abs_=True)[1])   
    
    if config['two_side']:
        n_temp = (temp-int(temp*config['temp2']))//2
        synth_samples = synth_samples[np.argsort(X_real_iat)][n_temp:-n_temp]
        real_samples = real_samples[n_temp:-n_temp]
    else:
        synth_samples = synth_samples[np.argsort(X_real_iat)][:int(temp*config['temp2'])]
        real_samples = real_samples[:int(temp*config['temp2'])]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ma", type=int, default=0
    )
    parser.add_argument(
        "--temp2", type=float, default=1
    )    
    parser.add_argument(
        "--two_side", type=int, default=0
    )    
