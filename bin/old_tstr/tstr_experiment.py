...
def main(config: dict, log_dir: str, samples_path: str):
    ...
    if os.path.exists(log_dir):
        import sys
        if os.path.isfile(log_dir / 'results.yaml'):
            import sys
...
if __name__ == "__main__":
    ...
    if args.schedule=='linear':
        ...
        args.out_dir = args.out_dir.replace('results',f"results_s{args.ckpt.split('_')[-1].split('.ckpt')[0]}")
    else:
        assert args.time_embed==0
        args.out_dir = args.out_dir.replace('results','results_wo_time')
        args.out_dir = args.out_dir.replace('results',f"results_{args.schedule}_{args.tau}_s{args.ckpt.split('_')[-1].split('.ckpt')[0]}")
    print(args.out_dir)
