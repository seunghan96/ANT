...
def sample_synthetic(
    ...
    for _ in tqdm(range(n_iters)):
        samples = model.sample_n(num_samples=batch_size,short_T=short_T)
...
def main(config: dict, log_dir: str, samples_path: str):
    ...
    if os.path.exists(log_dir):
        ...
        PATH = f'results_{scale}_seed{random_seed}.yaml'
        if config['mm']:
    ...
    if samples_path is None:
        ...
        synth_samples = sample_synthetic(model, num_samples=10000,short_T=config['short_T'])
        #scale = args.ckpt.split('_')[-1].split('.ckpt')[0]
        #np.save(log_dir / f"synth_samples_{scale}_seed{random_seed}_sT{config['short_T']}.npy", synth_samples)
        np.save(log_dir / f"synth_samples_sT{config['short_T']}.npy", synth_samples)
    else:
        try:
