...
def sample_synthetic(
    ...
    for _ in tqdm(range(n_iters)):
        samples = model.sample_n_viz(num_samples=batch_size)
        samples = np.array(samples)
        synth_samples.append(np.squeeze(samples,-1))

    SAVE_PATH = '/home/seunghan9613/tsdiff/VIZ_DIR'
    fn = 'temp2.npy'
    fn = os.path.join(SAVE_PATH,fn)
    np.save(fn, synth_samples)
    sys.exit(0)
